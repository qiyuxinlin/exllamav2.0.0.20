...
__forceinline__ __device__ void rope_cuda_arr_neox_inverse
(
    half* __restrict__ x,
    const half* __restrict__ sin,
    const half* __restrict__ cos,
...
)
{
    ...
    // Compute inverse sin and cos
    half2 inv_cos2_l = cos2_l;
    half2 inv_cos2_r = cos2_r;
    half2 inv_sin2_l = __hneg2(sin2_l);
    half2 inv_sin2_r = __hneg2(sin2_r);

    // Apply inverse embedding to row

    half2 item2_l = x_.item_half2(row_offset, column);
    half2 item2_r = x_.item_half2(row_offset, column + half_dim);
    half2 item2_ls = __hmul2(item2_r, inv_sin2_l);
    half2 item2_rs = __hmul2(item2_l, inv_sin2_r);
    item2_l = __hfma2(item2_l, inv_cos2_l, item2_ls);
    item2_r = __hfma2(item2_r, inv_cos2_r, item2_rs);
    x_.set_half2(row_offset, column, item2_l);
    x_.set_half2(row_offset, column + half_dim, item2_r);
}
...
__global__ void  rope_cuda_inverse_kernel
(
    half* __restrict__ x,
    const half* __restrict__ sin,
    const half* __restrict__ cos,
...
{
    if (neox_style)
        rope_cuda_arr_neox_inverse(x, sin, cos, rows_per_batch, head_dim, num_heads, past_len, past_lens, threads_y);
    else
        return;
}
...
)
{
    ...
    rope_cuda_inverse_kernel<<<gridDim, blockDim>>>
    (
        x,
        sin,
        cos,
