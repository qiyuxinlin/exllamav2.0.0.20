...
from exllamav2.cache import ExLlamaV2Cache_CPU
...
class ExLlamaV2:
    ...
    def load_autosplit_gen(self,
        ...
        with torch.inference_mode():
            ...
            if reserve_vram is None:
                # reserve_vram = [10240 * 1024**2] + [11240 * 1024**2] * (num_devices - 1)
                reserve_vram = [12240 * 1024**2] +  [11240 * 1024**2]* (num_devices - 1)
            ...
            past_len = 0
            attn_params = ExLlamaV2Attention.Params(batch_size, seq_len, past_len, None, None, None)
                    ...
                    try:
                        if isinstance(module, ExLlamaV2Attention) or \
                           isinstance(module, ExLlamaV2ParallelDecoder):
                            ...
                            if cache != None and not isinstance(cache,ExLlamaV2Cache_CPU):
    ...
    def select_index_forward(self,
        ...
        if cache is None or not isinstance(cache, ExLlamaV2CacheBase):
            ...
            result, last_state = self._select_index_forward(input_ids = input_ids,
                                               cache = cache,
                                               input_mask = input_mask,
                                               additional_attn_mask = additional_attn_mask,
                                               preprocess_only = preprocess_only,
        ...
        while chunk_begin < q_len:
            ...
            r, ls = self._select_index_forward(input_ids = input_ids[:, chunk_begin : chunk_end],
                                  cache = cache,
                                  input_mask = input_mask,
                                  additional_attn_mask = additional_attn_mask,
                                  preprocess_only = _preprocess_only,
    ...
    def cache_retrival_forward(self,
        ...
        while chunk_begin < q_len:
            ...
            _last_id_only = last_id_only
            _preprocess_only = preprocess_only or (chunk_end < q_len and last_id_only)
            kwargs['chunk_begin'] = chunk_begin
            kwargs['chunk_end'] = chunk_end
    ...
    def one_layer_forward(self,
        ...
        while chunk_begin < q_len:
            ...
            r, ls = self._one_layer_forward(input_ids = input_ids[:, chunk_begin : chunk_end],
                                  cache = cache,
                                  input_mask = input_mask,
                                  additional_attn_mask = additional_attn_mask,
                                  preprocess_only = _preprocess_only,
    ...
    def cache_blend_forward(self,
        ...
        if cache is None or not isinstance(cache, ExLlamaV2CacheBase):
            ...
            result, last_state = self._cache_blend_forward(input_ids = input_ids,
                                               cache = cache,
                                               input_mask = input_mask,
                                               additional_attn_mask = additional_attn_mask,
                                               preprocess_only = preprocess_only,
        ...
        while chunk_begin < q_len:
            ...
            # past_len = cache_full.current_seq_len
            past_len = cache.current_seq_len
            attn_size = (past_len + remaining_q_len) * remaining_q_len
            max_a = self.config.max_attention_size
            if attn_size > max_a:
            ...
            _last_id_only = last_id_only
            _preprocess_only = preprocess_only or (chunk_end < q_len and last_id_only)
            k_sub,v_sub,x_sub = self._cache_blend_forward(input_ids = input_ids[:, chunk_begin : chunk_end],
                                  cache_full = cache,
                                  past_len = past_len,
                                  input_mask = input_mask,
                                  additional_attn_mask = additional_attn_mask,
                                  ...
                                  last_layer=last_layer,
                                  **kwargs)
            if k_sub_all is None:
                k_sub_all = k_sub
                v_sub_all = v_sub
                x_sub_all = x_sub
            else:
                k_sub_all = torch.cat((k_sub_all, k_sub), dim = 1)
                v_sub_all = torch.cat((v_sub_all, v_sub), dim = 1)
                x_sub_all = torch.cat((x_sub_all, x_sub), dim = 1)


            chunk_begin = chunk_end
            remaining_q_len -= chunk_size      
        k_sub_all = k_sub_all.reshape(1,q_len,-1).squeeze(0)
        v_sub_all = v_sub_all.reshape(1,q_len,-1).squeeze(0)
    ...
    def _forward(self,
        ...
        x = input_ids
        attn_params = ExLlamaV2Attention.Params(batch_size, seq_len, past_len, input_mask, additional_attn_mask, position_offsets)
        last_state = None
        last_module = None
        flag2 = False
        for idx, module in enumerate(self.modules):
            ...
            # embedding + [attn MLP] + norm + linear
            # self.modules = self.config.num_hidden_layers * 2 + 3
            # if last_layer_flag == True and idx >= self.config.num_hidden_layers * 2 - last_layer * 2 and module.name == 'Attention':
            # if last_layer_flag == True and idx < 1 + last_layer * 2 and module.name == 'Attention':
            #     attn_params = ExLlamaV2Attention.Params(batch_size, seq_len, past_len, input_mask=input_mask, additional_attn_mask=whole_mask, position_offsets=position_offsets)
            #     last_layer_flag = False
            #     flag2 = True
            # elif flag2 == True and  idx >= 1 + last_layer * 2 and module.name == 'Attention':
            #     attn_params = ExLlamaV2Attention.Params(batch_size, seq_len, past_len, input_mask, additional_attn_mask, position_offsets)
            #     flag2 = False
            # else:
            #     attn_params.additional_attn_mask = additional_attn_mask
    ...
    @torch.inference_mode()
    def _select_index_forward(self,
                 input_ids: torch.Tensor,
                 cache: ExLlamaV2CacheBase | list[ExLlamaV2CacheBase] | None = None,
                 input_mask: torch.Tensor | None = None,
                 additional_attn_mask: torch.Tensor | None = None,
    ...
    @torch.inference_mode()
    def _cache_retrival_forward(self,
                 input_ids: torch.Tensor,
                 cache: ExLlamaV2CacheBase | list[ExLlamaV2CacheBase] | None = None,
                 input_mask: torch.Tensor | None = None,
                 additional_attn_mask: torch.Tensor | None = None,
    ...
    def _one_layer_forward(self,
        ...
        for idx, module in enumerate(self.modules):
            ...
            if preprocess_only and idx == self.last_kv_layer_idx:
                x = None
                break
            if module.name == 'Attention' and idx == 3:
                break
    ...
    @torch.inference_mode()
    def _cache_blend_forward(self,
                 input_ids: torch.Tensor,
                 cache_full: ExLlamaV2CacheBase | list[ExLlamaV2CacheBase] | None = None,
                 cache_change: ExLlamaV2CacheBase | list[ExLlamaV2CacheBase] | None = None,
        ...
        for idx, module in enumerate(self.modules):
            ...
            x = safe_move_tensor(x, device)
            if idx <= 3 and module.name == 'Attention':
                # 全mask算一遍
                attn_params = ExLlamaV2Attention.Params(batch_size, seq_len, past_len, input_mask=input_mask, additional_attn_mask=whole_mask, position_offsets=position_offsets)
                if(x_full == None):
                    x_full = x.clone()
                else:
                    x_full = safe_move_tensor(x_full, device)
                x_full = module.forward(x_full, cache = cache_full, attn_params = attn_params, past_len = past_len, loras = loras, **kwargs)
                k_cache_full, v_cache_full = cache_full.get_kv_state((idx-1)//2,batch_size, 0, past_len)
                k_cache_full = k_cache_full.narrow(0, 0, batch_size).narrow(1, past_len, seq_len).clone()
                v_cache_full = v_cache_full.narrow(0, 0, batch_size).narrow(1, past_len, seq_len).clone()
                # 改mask算一遍
                attn_params = ExLlamaV2Attention.Params(batch_size, seq_len, past_len, input_mask, additional_attn_mask, position_offsets)
                if(x_change == None):
                    x_change = x.clone()
                else:
                    x_change = safe_move_tensor(x_change, device)
                x_change = module.forward(x_change, cache = cache_full, attn_params = attn_params, past_len = past_len, loras = loras, **kwargs)
                k_cache_change,v_cache_change = cache_full.get_kv_state((idx-1)//2,batch_size, 0, past_len)
                # k_cache_change,v_cache_change = cache_change.get_kv_state((idx-1)//2,batch_size, 0, past_len)
                k_cache_change = k_cache_change.narrow(0, 0, batch_size).narrow(1, past_len, seq_len)
                v_cache_change = v_cache_change.narrow(0, 0, batch_size).narrow(1, past_len, seq_len)
                # 比较哪些token变化较大
                if idx == 1:
                    k_tmp = torch.abs(k_cache_change - k_cache_full)
                    v_tmp = torch.abs(v_cache_change - v_cache_full)
                    x_tmp = torch.abs(x_change - x_full)
                    cache_full.current_seq_len += seq_len
                    # cache_change.current_seq_len += seq_len
                    # k_tmp = k_tmp.reshape(1,seq_len,-1).squeeze(0)
                    # k_sum = torch.sum(k_tmp,dim=1)
                    # k_need_index = torch.topk(k_sum,int(seq_len*rate)).indices
                    # v_tmp = v_tmp.reshape(1,seq_len,-1).squeeze(0)
                    # v_sum = torch.sum(v_tmp,dim=1)
                    # v_need_index = torch.topk(v_sum,int(seq_len*rate)).indices
                    return k_tmp,v_tmp,x_tmp
                
                x = x_full
            else:
                x = module.forward(x, cache = cache_full, attn_params = attn_params, past_len = past_len, loras = loras, **kwargs)
            if preprocess_only and idx == self.last_kv_layer_idx:
                x = None
                break
