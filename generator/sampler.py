...
class ExLlamaV2Sampler:
    ...
    @staticmethod
    def nbce_sample(logits: torch.tensor,
               settings: Settings,
               sequence_ids: torch.tensor,
               random: float,
               tokenizer: ExLlamaV2Tokenizer,
        ...
        # 加入nbce修改
            ...
            probas = torch.nn.functional.softmax(logits[None] / settings.temperature , dim=-1)
            next_tokens = torch.multinomial(probas, num_samples=1).squeeze(1) 
            output_tokens = torch.full((batch_size, 1),next_tokens[0], device = "cpu", dtype = torch.long)
            output_probs = torch.empty((batch_size, 1), device = "cpu", dtype = torch.float)   
            # Stop condition from filters

            end_filter = False
            # print(f'{tokenizer.decode(output_tokens[0])}, {output_tokens[0]}')
            # if len(settings.filters) > 0 and output_tokens[0].item() == stop_token: end_filter = True
