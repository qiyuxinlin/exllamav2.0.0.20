...
class ExLlamaV2Cache_CPU(ExLlamaV2CacheBase):
    """
    FP16 cache
    """
    ...
    def __init__(self,
                 ...
                 batch_size: int = 1,
                 max_seq_len: int = -1,
                 copy_from: ExLlamaV2Cache_CPU | None = None,
                 lazy: bool = False):
    ...
    def create_state_tensors(self,
        ...
        assert copy_from is None and lazy == False, "only supprot lazy == False"
        if not lazy:
            for i in range(self.num_hidden_layers):
                # device = self.model.cache_map[i]
                device = 'cpu'
    ...
    def get_kv_state(self,
        ...
        return self.key_states[layer_idx].to(self.model.cache_map[layer_idx]), self.value_states[layer_idx].to(self.model.cache_map[layer_idx])


    
    def store_kv_state(self, layer_idx , batch_size, all_len, key_states, value_states):
        ss = torch.cuda.current_stream()
        with torch.cuda.stream(self.copy_stream):
            torch.cuda.current_stream().wait_stream(ss)
            self.key_states[layer_idx][:batch_size,0:all_len,:,:] = key_states.to('cpu')
            self.value_states[layer_idx][:batch_size,0:all_len,:,:] = value_states.to('cpu')
